...
**This repository’s copy** under `agentskills/jira-epic-check/` is **Markdown-only**: **`SKILL.md`**, **`references/CONTEXT.md`** (org vocabulary ↔ List 1 / List 2), **`references/INSTRUCTIONS.md`** (parameterized project + **`LIST1_DAYS`** / **`LIST2_DAYS`**), and **`references/evaluation.md`** (recorded MCP verification run) — **no `scripts/`** and **no non-`.md` files** in that skill folder.
